// Two Pass assembler small interpretation
// @MiteshNinja 16/04/16
// Read http://www.lysator.liu.se/c/c-faq/c-faq-toc.html#c-2
// and c-faq.com for a better understading of the concepts!
...
int find_symbol(char *token, struct table tbl[], int limit);
...
int main(int argc, char* argv[])
{
    ...
    while (fgets(line, sizeof line, fp) != NULL) {
        if((temp = flag[line_count]) != -1){
            ...
            temp = find_symbol(tempchar, st, next_st_index);
...
void table_init(struct table tbl[TABLE_SIZE])
{
    int i;
...
int find_symbol(char *token, struct table tbl[TABLE_SIZE], int limit)
{
    int i;
    for(i = 0; i < limit; i++){
...
void process_token(char *token, int first)
{
    ...
    if(checkTok(token, pot) != -1){
        ...
        if(first && (find_symbol(token, st, next_st_index) == -1)){
            ...
            // st[next_st_index++].location = line_count;
        } else {
            // If it's not the firstword.
            if((temp = find_symbol(token, st, next_st_index)) != -1){
                result[line_count] += temp;
            } else{
                if((temp = find_symbol(token, ft, next_ft_index)) == -1){
                    // If it's not present in flag table
                    // add it to flag table
                    strcpy(ft[next_ft_index].symbol, token);
                    ft[next_ft_index].location = next_ft_index++;
                }
                flag[line_count] = find_symbol(token, ft, next_ft_index);
            }
        }
    }
}
